...
CREATE POLICY "Users manage own upload-post profile" ON public.upload_post_profiles
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Service role manages upload-post profiles" ON public.upload_post_profiles
...
CREATE POLICY "Users manage own social accounts" ON public.social_accounts
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Service role manages social accounts" ON public.social_accounts
  FOR ALL TO service_role USING (true) WITH CHECK (true);
...
CREATE TABLE IF NOT EXISTS public.publishing_jobs (
  ...
  title TEXT,
  caption_per_platform JSONB NOT NULL DEFAULT '{}',
...
CREATE POLICY "Users manage own publishing jobs" ON public.publishing_jobs
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Service role manages publishing jobs" ON public.publishing_jobs
  FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE INDEX IF NOT EXISTS idx_publishing_jobs_user ON public.publishing_jobs(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_publishing_jobs_status ON public.publishing_jobs(status);
CREATE INDEX IF NOT EXISTS idx_publishing_jobs_scheduled ON public.publishing_jobs(scheduled_at) WHERE status = 'scheduled';
CREATE INDEX IF NOT EXISTS idx_publishing_jobs_request ON public.publishing_jobs(upload_post_request_id);
...
CREATE POLICY "Users view own publishing results" ON public.publishing_results
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Service role manages publishing results" ON public.publishing_results
  FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE INDEX IF NOT EXISTS idx_publishing_results_job ON public.publishing_results(job_id);
CREATE INDEX IF NOT EXISTS idx_publishing_results_user ON public.publishing_results(user_id);
...
CREATE POLICY "Users view own analytics" ON public.analytics_snapshots
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Service role manages analytics snapshots" ON public.analytics_snapshots
  FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE INDEX IF NOT EXISTS idx_analytics_user_platform ON public.analytics_snapshots(user_id, platform, snapshot_date DESC);
...
CREATE POLICY "Users view own post analytics" ON public.post_analytics
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Service role manages post analytics" ON public.post_analytics
  FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE INDEX IF NOT EXISTS idx_post_analytics_user ON public.post_analytics(user_id, last_fetched_at DESC);
...
CREATE TRIGGER trg_upload_post_profiles_updated BEFORE UPDATE ON public.upload_post_profiles
